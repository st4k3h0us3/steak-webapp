...
const WalletInfo: FC = () => {
  ...
  const address = useStore((state) => state.wallet?.accAddress);
  const uusd = useStore((state) => state.balances?.uusd);
          ...
          <Flex color="white" justify="center">
            ...
            >
              <HStack spacing="3">
                ...
                <Text fontSize="md" color="white">
                  {uusd ? formatNumber(uusd / 1e6) : "0.00"}
                </Text>
              </HStack>
            </Center>
          </Flex>
    ...
    >
      <Flex direction="column" justify="center" px="2">
        ...
        <Flex mt={3} justify="left" verticalAlign="middle">
          <Button onClick={() => copy(address!)} variant="simple">
            <HStack>
          ...
          <Link
            ...
            my="auto"
            isExternal
            href={`https://terrasco.pe/${network}/address/${address}`}
